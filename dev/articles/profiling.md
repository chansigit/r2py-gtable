...
- **`data.frame` construction and indexing.** gtable now includes a
  ...
  calling `.subset2`
- **Input validation.**
  [`stopifnot()`](https://rdrr.io/r/base/stopifnot.html) was identified
  as a bottleneck and has removed in favor of a standard
  `if (...) stop()`
- **Dimension querying.** The use of
  [`nrow()`](https://rdrr.io/r/base/nrow.html) and
  [`ncol()`](https://rdrr.io/r/base/nrow.html) has internally been
  substituted for direct calls to
  [`length()`](https://rdrr.io/r/base/length.html) of the `heights` and
  `widths` unit vectors
